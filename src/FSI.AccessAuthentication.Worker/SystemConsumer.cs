...
{
    public class SystemConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _config;

        public SystemConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        {
            string queueName = "System-queue";

            var factory = new ConnectionFactory
            {
                HostName = _config["RabbitMq:Host"] ?? "localhost",
                SystemName = _config["RabbitMq:System"] ?? "guest",
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<SystemMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<ISystemAppService>();
                    ...
                    IEnumerable<SystemDto> listSystem = null;

                    SystemDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "insert":
                            ...
                            break;
                        case "getall":
                            listSystem = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listSystem, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, SystemMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<SystemDto> listSystem, SystemDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listSystem);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, SystemMessage envelope,
            string queueName, long? createdId)
        {
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    MessageResponse = string.Empty,
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert System into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
                });
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, SystemMessage envelope,
            string queueName, IEnumerable<SystemDto> listSystem)
        {
            if (listSystem != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listSystem);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listSystem),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert System into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, SystemMessage envelope,
            string queueName, SystemDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, SystemMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, SystemMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
